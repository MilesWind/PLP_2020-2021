...
def load_data():
    ...
    for folder in os.listdir(directory):
        ...
        if foldername == '.DS_Store':
            continue;
        for file in os.listdir(os.fsencode('Training/'+foldername)):
            filename = os.fsdecode(file);
            if filename == '.DS_Store':
                continue;

            try:
                image = load_img("Training/"+foldername+"/"+filename);
...
class GAN(object):
    ...
    def generator(self):
        ...
        self.G.add(Dense(int(self.img_rows/64)*int(self.img_cols/64)*self.depth, input_dim=100));
        self.G.add(BatchNormalization(momentum=0.9));
        self.G.add(Activation('relu'));
        self.G.add(Reshape((int(self.img_rows/64), int(self.img_cols/64), self.depth)));
    ...
    def discriminator_model(self):
        if self.DM:
            return self.DM;
        optimizer = RMSprop(learning_rate=0.02, clipvalue=1.0, decay=0);
        self.DM = Sequential();
...
class Flower_GAN:
    ...
    def Train(self, train_steps, data_width, save_interval):
